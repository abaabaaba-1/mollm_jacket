...
from model.util import nsga3_selection


class NSGA3BaselineMOO(BaselineMOO):

    def __init__(self, reward_system, llm, property_list, config, seed):
        super().__init__(reward_system, llm, property_list, config, seed)
        print("--- NSGA-III Baseline MOO has been activated (inherits ENHANCED BaselineMOO) ---")

    def select_next_population(self, pop_size: int) -> List:
        """NSGA-III style environmental selection on the archive buffer."""
        whole_population = [item[0] for item in self.mol_buffer if item[0].total is not None]
        if not whole_population:
            return []
        return nsga3_selection(whole_population, pop_size)


class NSGA3BaselineRunner(MOLLM):
    def run(self):
        if self.resume:
            self.load_from_pkl(self.save_path)
        moo = NSGA3BaselineMOO(self.reward_system, self.llm, self.property_list, self.config, self.seed)
        ...
        print("\nNSGA-III baseline run finished.")


def main():
    parser = argparse.ArgumentParser(description='Run NSGA-III Baseline (config-driven).')
    ...
    config.config['save_suffix'] = f"{original_suffix}_baseline_NSGA3"
    ...
    baseline_runner = NSGA3BaselineRunner(config=config, resume=args.resume, eval=False, seed=args.seed)
    baseline_runner.run()
